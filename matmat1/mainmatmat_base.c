...
#define NSTART 256
#define NSTEP 256
#define BASEFILENAME "base"
#define BASEFILE "base.csv"
...
void exec_matmatbase(int lda, int ldb, int ldc,
                     double *A, double *B, double *C,
                     int N1, int N2, int N3,
                     void (*matmat_func)(int, int, int,
                                         ...
                                         int, int, int))
{
  ...
  t1 = get_cur_time();
  matmat_func(lda, ldb, ldc, A_copy, B_copy, C_copy, N1, N2, N3);
  t2 = get_cur_time();
...
void plot_results()
{
  ...
  if ((ret = system("python3 plot_base.py " BASEFILE " -c ikj,block")) == -1)
  {
    perror("system");
    exit(1);
  }
  ...
  {
    fprintf(stderr, "xdg-open terminato con codice %d\n", ret);
  }

  if ((ret = system("python3 plot_base.py " BASEFILE " -c ikj,block -o plot_" BASEFILENAME "_2.png")) == -1)
  {
    perror("system");
    exit(1);
  }
...
int main()
{
  ...
  double *A, *B, *C;
  double get_cur_time(), t1, t2;
  void matmatikj(int, int, int,
                 double *, double *, double *,
                 int, int, int),
      matmatijk(int, int, int,
                double *, double *, double *,
                int, int, int),
      matmatjki(int, int, int,
                double *, double *, double *,
                int, int, int),
      matmatjik(int, int, int,
                double *, double *, double *,
                int, int, int),
      matmatkij(int, int, int,
                double *, double *, double *,
                int, int, int),
      matmatkji(int, int, int,
                double *, double *, double *,
                int, int, int);
  ...
  fprintf(f, "N,kji,jki,ijk,jik,ikj,kij,block\n");
  fclose(f);
  ...
  for (int i = NSTART; i <= NMAX; i += NSTEP)
  {
    ...
    printf("matmatkji\n");
    exec_matmatbase(lda, ldb, ldc, A, B, C, N1, N2, N3, matmatkji);
    printf("matmatjki\n");
    exec_matmatbase(lda, ldb, ldc, A, B, C, N1, N2, N3, matmatjki);
    printf("matmatijk\n");
    exec_matmatbase(lda, ldb, ldc, A, B, C, N1, N2, N3, matmatijk);
    printf("matmatjik\n");
    exec_matmatbase(lda, ldb, ldc, A, B, C, N1, N2, N3, matmatjik);
    printf("matmatikj\n");
    exec_matmatbase(lda, ldb, ldc, A, B, C, N1, N2, N3, matmatikj);
    printf("matmatkij\n");
    exec_matmatbase(lda, ldb, ldc, A, B, C, N1, N2, N3, matmatkij);
    printf("matmatblock\n");
